...
#            detect()
    ...
    config = ConfigParser()
    config.read('detect-yoshi.config')
    ...
    while True:
        foundYoshi = False
        os.system('timeout 10s ffmpeg -re -y -loglevel fatal -rtsp_transport tcp -i ' + video_url + ' -r 0.25 -frames:v 1 frame.jpg > /dev/null 2>&1 < /dev/null')
        time.sleep(0.2)

        now = datetime.now()
        logString = now.strftime("%d-%m-%Y-%H:%M:%S")

        filePath = opt.project + '/' + opt.name
        ...
        if foundYoshi:
            ...
            #os.system('mpack -s "sub: attachment" inference/output/frame.jpg don@munroe.ca')
